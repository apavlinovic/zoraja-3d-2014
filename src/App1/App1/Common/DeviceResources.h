...
namespace DX
{
	...
	private:
		void SetRasterizerState(D3D11_FILL_MODE fill, D3D11_CULL_MODE cull, bool fccw)
		{
			...
			Microsoft::WRL::ComPtr<ID3D11RasterizerState1> state;
			m_d3dDevice->CreateRasterizerState1(&desc, state.GetAddressOf());

			m_d3dContext->RSSetState(state.Get());
		}
